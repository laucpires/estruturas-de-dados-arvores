...
/*
Atualização de Alturas: Após cada operação de remoção, as alturas dos nós são
recalculadas

Balanceamento Automático: Chamada do método balance() após qualquer modificação
na estrutura

Rotações Duplas: Necessárias em casos específicos de desbalanceamento complexo

Propagação Recursiva: O balanceamento é feito de forma recursiva desde o ponto
de remoção até a raiz
*/
